...
public class WordBreak_LC139 {
    ...
    //{N: no. Of Strings in list, I: Average length of String, L: Length of string that need to check for word break}
    class Trie{
        TrieNode root;
        Trie(){
            root = new TrieNode('#');
        ...
        private boolean isWordBreak(String word){
            ...
            for(int i=0; i<size; i++){
                //If it is not word break, we will continue to search character which can be break at point.
                if(!wordBreak[i]){
                    continue;
                }
